...
/**
 * Does this error mean "the component is not there YET" — the one condition that justifies
 * walking further up?
 *
 * Everything else (EACCES, ELOOP, EPERM, ENAMETOOLONG, EIO) means we could not PROVE where the
 * path lands. Treating those as "absent" and re-attaching the tail hands back a path that was
 * never validated, through components we were unable to inspect. A confinement primitive that
 * cannot prove must REFUSE.
 */
