...
gsutil cp /var/log/scoreboard.txt "\$GCS/bench/board-$MODEL${RUN_TAG:+-$RUN_TAG}.txt" || true
...
[ -n "\$LATEST_T" ] && gsutil cp "\$LATEST_T" "\$GCS/bench/transcript-$MODEL${RUN_TAG:+-$RUN_TAG}.jsonl" || true
